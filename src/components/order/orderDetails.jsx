...
const Order = ({ match }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      axios
        .get(`${process.env.REACT_APP_API_URL}/v3/order/single?id=${id}`)
        .then((data) => {
        ...
        <div className="order-page">
          <div className="cart-item-container">
            <div className="order-list-head">
              {" "}
              <div className="order_details_num_head">
                <p>ORDER NUMBER</p>
              </div>
              <div className="order_details_mtd_head">
                <p>PAYMENT METHOD</p>
              </div>
              <div className="order_details_status_head">
                <p>STATUS</p>
              </div>
              <div className="order_details_price_head ">
            ...
            <div className="single-order-list">
              <div className="order-list-item">
                ...
                <div className="order_num">
                  <p className="ord_number">{order[0]?.order_number}</p>
                </div>
                <div className="order_mtd">
                  <p>{order[0]?.payment_method}</p>
                </div>
                <div className="order_status">
                  <p className="order_status_details">{order[0]?.status}</p>
                </div>
                <div className="order_price">
                  <p className="order-delivery-add">
                    {" "}
                    <span className="delivery-tag">Delivery Area</span>
                    {order[0]?.delivery_address}
                  </p>
                </div>
                <div className="order_price">
                  <p className="order-delivery-price">
                    {" "}
                    <span className="price-tag">Delivery Price</span>2000
                  </p>
                </div>
                <div className="order_price">
              ...
              <div className="item-list-head">
                {" "}
                <div className="item_num_head">
                  <p>ITEM IMAGE</p>
                </div>
                <div className="item_mtd_head">
                  <p>ITEM NAME</p>
                </div>
                <div className="item_status_head">
                  <p>ITEM QTY</p>
                </div>
                <div className="item_price_head ">
                  <p>ITEM PRICE</p>
                </div>
              </div>

              <div className="item-details-">
                {order[0]?.items?.map((details) => (
                  <div key={details._id}>
                    <div className="item-details">
                      ...
                      <p className="id-name">{details.name}</p>
                      <p className="id-qty">{details.quantity}</p>
                      <p className="id-price">{details.price}</p>
                    </div>
                  </div>
                ))}
              </div>
